...
impl ElementExtractor {
    ...
    pub fn select_elements(&self, selector: &str) -> Result<Vec<Value>> {
        ...
        let elements: Vec<Value> = self.document
            .select(&css_selector)
    ...
    pub fn extract_text(&self, selector: &str) -> Result<Vec<String>> {
        ...
        let texts: Vec<String> = self.document
            .select(&css_selector)
    ...
    pub fn extract_attributes(&self, selector: &str, attribute: &str) -> Result<Vec<String>> {
        ...
        let attributes: Vec<String> = self.document
            .select(&css_selector)
    ...
    pub fn extract_links(&self) -> Result<Vec<Value>> {
        let links = self.select_elements("a[href]")?
            .into_iter()
            .map(|mut link| {
    ...
    pub fn extract_images(&self) -> Result<Vec<Value>> {
        let images = self.select_elements("img")?
            .into_iter()
            .map(|mut img| {
    ...
    pub fn extract_forms(&self) -> Result<Vec<Value>> {
        ...
        let forms: Vec<Value> = self.document
    ...
    pub fn extract_tables(&self) -> Result<Vec<Value>> {
        ...
        let header_selector = Selector::parse("thead tr th, tr:first-child th, tr:first-child td").unwrap();
        let row_selector = Selector::parse("tbody tr, tr").unwrap();
        let cell_selector = Selector::parse("td, th").unwrap();
        
        let tables: Vec<Value> = self.document
    ...
    pub fn extract_metadata(&self) -> Value {
        let title = self.extract_text("title").unwrap_or_default().first().cloned().unwrap_or_default();
        let description = self.extract_attributes("meta[name='description']", "content")
            .unwrap_or_default().first().cloned().unwrap_or_default();
...
impl FormSubmitter {
    ...
    pub async fn submit_form(&mut self, form_action: &str, method: &str, data: HashMap<String, String>) -> Result<String> {
